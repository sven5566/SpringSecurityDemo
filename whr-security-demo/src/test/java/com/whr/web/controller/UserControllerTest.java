...
public class UserControllerTest {
	...
	public void whenQuerySuccess() throws Exception {
		String resultString = mockMvc
				.perform(get("/user").param("userName", "we").param("age", "15").param("ageTo", "34")
						// .param("size", "4")
						// .param("number", "2")
						// .param("sort", "age DESC")
						.contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(3)).andExpect(status().isOk())
	...
	public void whenGetUserInfoSuccess() throws Exception {
		String resultString = mockMvc.perform(get("/user/1").contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(jsonPath("$.userName").value("tom")).andExpect(status().isOk()).andReturn().getResponse()
				.getContentAsString();
